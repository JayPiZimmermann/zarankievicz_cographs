...
\begin{abstract}
A \emph{cograph} is a graph that contains no induced path $P_4$ on four vertices.
...
Motivated by the rarity of complete classification of extremal configurations, we completely classify all $K_{3,3}$-free extremal cographs by proof and all $K_{s,t}$-free extremal cographs for $s \leq t \leq \text{min} \{ 12, 2s-1 \}$ by computationally analyzing the respective cores using an algorithm based on dynamic programming.
\end{abstract}
...
\end{align*}
    for all large $n$ and $\alpha = \frac{t-1}{2} + s - 1$ in case $t \leq 2s - 1$.
...
It is clear that $(1-t)$-extremal cographs are cographs of maximal degree $t-1$.
...
\begin{Lemma}[Forbidden stars] \label{thm:pumping_stars}
    Let $t,n \in \NN$ with $n \geq t$. In case $n$ is even or $t-1$ are even and $2t \neq n+1$, any $(s,t)$-extremal graph on $n$ vertices is $(t-1)$-regular.
...
\begin{Theorem}[Pumping Theorem] \label{thm:pumping}
    ...
    Moreover, in case that $t \leq 2 s-1$, the pumping component $H_j$ has a common neighbourhood in $G_j$ of size $s-1$ and the pumping components are $(t-1)$-regular, i.e. $\alpha = \frac{t-1}{2} + s-1$. Moreover in this case we might find choose $R = t$ and there is at least one $g \in \mathbb{Z} / \mathbb{Z}$ such that for large $n$ with $n \ \text{mod} t = g$ the $(s,t)$-extremal cograph on $n$ vertices is connected.
\end{Theorem}
...
\begin{proofref}{Theorem}{thm:pumping}
    ...
    \paragraph{5. The pumping structure in case $t \leq 2s - 1$:}
    ...
    Hence, we know that $|W| = s-1$. We know that there is $Q = G(u_j)$ and $H_0$ and $H_1$ such that
    \begin{align}
        G_j = H_0 + G[W] \times \left( H_{1} + Q \right) 
    \end{align}
    By Lemma \ref{thm:regular_cographs} is clear, that $Q$ is $(t-1)$-regular.

    It is clear that $G[W]$ is extremal with respect to some biclique profile $\mathscr{P}$ that depends on $H_1 + Q$. It is clear that adding a $t$-clique to $\left( H_1 + Q \right)$ does not change $\mathscr{P}$ as $(s-1) + t < |K_{s,t}|$.

    As $K_t$ is $(t-1)$-regular we may assume that $Q = K_t$ and that $R = t$.

    It is clear that any component of $H_1$ has a maximum degree at most $t-1$ and therefore has a size at most $2(t-1)$. 
    In order to reach a certain vertex count modulo $t$, $t-1$ components in $H_1$ suffice. We do not know that $H_1$ is $(t-1)$-regular as it could bring more edges to have less edges in $H_1$ resulting in a less restrictive profile $\mathscr{P}$.
    By the given arguments observe that
    \begin{align}
        |H_1| \leq 2 (t-1)^2.
    \end{align}

    Moreover, we know that $e(H_0) < \alpha$.
    Assume for a contradiction that $e(H_i) \geq \alpha$.
    We might pump up $H_i$ instead of $Q$ by factors that are multiples of $R$ and obtain extremal graphs with arbitrary many connectivity components. Similarly to before, this is a contradiction to the Lifting Theorem \ref{thm:lifting}.
      
    Assume now for a contradiction that $|H_0| \geq \binom{s-1}{2} + 2$.
    
    We might replace $H_0$ by the empty graph and $G[W]$ by $E_{w}$ and $Q$ by $Q'$.
    Here, $Q'$ on $|Q| + |H_1|$ is constructed using Lemma \ref{thm:regular_cographs} as either a $(t-1)$-regular cograph or as the sum of a singleton and a $(t-1)$-regular cograph. Let us again call the resulting graph $G'$.
    We know that
    \begin{align}
        \| Q' \| \geq \left( |Q| + |H_0| - 1 \right) \cdot \alpha \label{eq:extremal_structure}
    \end{align}
    We deleted at most $\sum_{i+1 \leq k \leq l} \binom{|W_k|}{2} \leq \binom{s-1}{2}$ edges from $G[W]$ but gained at least $\left(|H_0| - 1\right) (\alpha - e(H_j)) \geq |H_0| - 1 \geq \binom{s-1}{2} + 1$ edges, a contradiction to edge-maximality.

    It is clear that in case we pump up $Q$ large enough, the resulting cograph $G[W] \times \left( H_1 + z \cdot Q \right)$ is extremal itself and for $n \ \text{mod} \ t = |W| + |H_0| \ \text{mod} \ t$ we may choose $G_j = G[W] \times H_0$, i.e. the extremal cograph for such $n$ is connected.    
\end{proofref}

\begin{Theorem} \label{thm:small_cases_classification}
    For $s,t \in [12]$ with $s \leq t \leq 2s - 1$ and $n \geq 2t - 1$ any $(s,t)$-extremal cograph on $n$ vertices has structure $G_0 \times G_1$ where $|G_0| = s-1$ and $G_1$ is $(1,t)$-extremal.
\end{Theorem}

\begin{proofref}{Theorem}{thm:small_cases_classification}
    It is clear from the proof of Theorem \ref{thm:pumping} that for large $n$ there is an $(s,t)$-extremal cograph $G$ on $n$ vertices with the following structure
    \begin{align}
        G = H_0 + G_0 \times \left( H_1 + k \cdot K_t \right)
    \end{align}

    Here, $H_0$ is an extremal cograph on at most $\binom{s-1}{2} + 1$ vertices, $G_0$ is a $\mathscr{P}^{(G_0)}$-extremal cograph on $s-1$ vertices for some biclique-profile $\mathscr{P}^{(G_0)}$ and any of the at most $t-1$ connectivity components of $H_0$ is a $\mathscr{P}^{(H_0)}$-extremal cograph on at most $2(t-1)$ vertices for some biclique-profile $\mathscr{P}^{(H_0)}$.

    Hence, to check that $H_0$ is the graph on empty vertex set, it suffices to check the edge density of all such possible triples $\left(H_0, G_0, H_1\right)$ such that $G_0 \times H_0$ does not contain a $K_{s,t}$. To find the edge density of possible $H_0$ we might use Algorithm \ref{alg:dynamic_programming} and a version of it to find $H_1$ and $G_0$, where we only search for $\mathscr{P}$-extremal cographs for all $\mathscr{P}$ that could arise on small vertex count.

    For any such found triple $\left(H_0, G_0, H_1\right)$ we consider $m \coloneq |H_0| + s-1 + |H_1|$ and $q \coloneq m \ \text{mod} \ t$ and we consider the edge-count $\left\| H_0 + G_0 \times \left(H_0 + \frac{N-m-q}{t} \cdot K_t \right) \right\|$ for $N \geq \binom{s-1}{2} + s + 2(t-1)^2$ with $N \ \text{mod} \ t = 0$.

    Then, the extremal graphs for large $n$ would consist of those triples that maximize this edge-count for a specific $q$.

    When we performed the algorithm for all the specified parameters $s$ and $t$ we observed that in the maximizing triples $H_0$ is the graph on the empty vertex set and $H_1$ is $(1,t)$-extremal.
    This closes the proof of Theorem \ref{thm:small_cases_classification}.    
\end{proofref}
...
An interesting open question is whether the restriction $t \le 2s-1$ used in the Lifting Theorem is necessary for the finite component decomposition, or if it is an artifact of the proof method. We assume that the statement of Theorem \ref{thm:small_cases_classification} holds for arbitrary $s,t$ and $n$.
